...
if [ -n "$GITHUB_REF" ] && [[ "$GITHUB_REF" == refs/tags/* ]]; then
    # If running in GitHub Actions with a tag
    tag="${GITHUB_REF#refs/tags/}"
else
    # Try to get the latest tag or use version from Cargo.toml
    tag="$(git describe --tags --abbrev=0 2>/dev/null)"
    if [ -z "$tag" ]; then
        # Extract version from Cargo.toml and create tag
        version=$(grep '^version = ' Cargo.toml | sed 's/version = "\(.*\)"/\1/')
        tag="v$version"
    fi
fi

# Make sure the tag could be determined.
if [ -z "$tag" ]; then
    printf "\e[31mError: Unable to determine git tag\e[0m\n"
    exit 1
fi
